...
const SupplierList = () => {
  ...
  const totalItem = useSelector((state) => state.supplier.totals);
  ...
  useEffect(() => {
    (async () => {
      await SupplierListRequest(page, perPage, searchKey);
    })();
  }, []);
  ...
  const perPageChange = async (e) => {
    ...
    await SupplierListRequest(page, e.target.value, searchKey);
  };
  ...
  const paginateOnChange = async (e) => {
    await SupplierListRequest(e.selected + 1, perPage, searchKey);
  };

  const deleteItem = (id) => {
    Swal.fire({
      title: "Delete Supplier",
      text: "Do you want to delete this supplier info?",
      ...
      if (result.isConfirmed) {
        return SupplierDeleteRequest(id).then(async (res) => {
          if (res) {
            await SupplierListRequest(page, perPage, searchKey);
          }
        });
