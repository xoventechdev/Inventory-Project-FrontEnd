...
const ExpenseList = () => {
  ...
  const totalItem = useSelector((state) => state.expense.totals);
  ...
  useEffect(() => {
    (async () => {
      await ExpenseListRequest(page, perPage, searchKey);
    })();
  }, []);
  ...
  const perPageChange = async (e) => {
    ...
    await ExpenseListRequest(page, e.target.value, searchKey);
  };
  ...
  const paginateOnChange = async (e) => {
    await ExpenseListRequest(e.selected + 1, perPage, searchKey);
  };
  ...
  const deleteItem = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        return ExpenseDeleteRequest(id).then(async (res) => {
          if (res) {
            await ExpenseListRequest(page, perPage, searchKey);
          }
        });
