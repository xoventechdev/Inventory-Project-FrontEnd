...
const ReturnList = () => {
  const tableItem = useSelector((state) => state.return.list);
  const totalItem = useSelector((state) => state.return.totals);
  ...
  useEffect(() => {
    (async () => {
      await ReturnListRequest(page, perPage, searchKey);
    })();
  }, []);
  ...
  const perPageChange = async (e) => {
    ...
    await ReturnListRequest(page, e.target.value, searchKey);
  };
  ...
  const paginateOnChange = async (e) => {
    await ReturnListRequest(e.selected + 1, perPage, searchKey);
  };

  const deleteItem = (id) => {
    Swal.fire({
      title: "Delete Return",
      text: "Do you want to delete this return info?",
      ...
      if (result.isConfirmed) {
        return ReturnDeleteRequest(id).then(async (res) => {
          if (res) {
            await ReturnListRequest(page, perPage, searchKey);
          }
        });
