...
const BrandList = () => {
  ...
  const totalItem = useSelector((state) => state.brand.totals);
  ...
  const paginateOnChange = async (e) => {
    await BrandListRequest(e.selected + 1, perPage, searchKey);
  };

  const deleteItem = (id) => {
    Swal.fire({
      title: "Delete Brand",
      text: "Do you want to delete this brand info?",
      ...
      if (result.isConfirmed) {
        return BrandDeleteRequest(id).then(async (res) => {
          if (res) {
            await BrandListRequest(1, perPage, searchKey);
          }
        });
  ...
  const changeStatus = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        return BrandStatusChange(id).then(async (res) => {
          if (res) {
            await BrandListRequest(1, perPage, searchKey);
          }
        });
