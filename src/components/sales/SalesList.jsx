...
const SalesList = () => {
  const tableItem = useSelector((state) => state.sales.list);
  const totalItem = useSelector((state) => state.sales.totals);
  ...
  useEffect(() => {
    (async () => {
      await SalesListRequest(page, perPage, searchKey);
    })();
  }, []);
  ...
  const perPageChange = async (e) => {
    ...
    await SalesListRequest(page, e.target.value, searchKey);
  };
  ...
  const paginateOnChange = async (e) => {
    await SalesListRequest(e.selected + 1, perPage, searchKey);
  };

  const deleteItem = (id) => {
    Swal.fire({
      title: "Delete Sales",
      text: "Do you want to delete this sales info?",
      ...
      if (result.isConfirmed) {
        return SalesDeleteRequest(id).then(async (res) => {
          if (res) {
            await SalesListRequest(page, perPage, searchKey);
          }
        });
